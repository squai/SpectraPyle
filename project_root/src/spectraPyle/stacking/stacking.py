...
class Stacking:
    ...
    def run(self):    
        ...
        if self.config['pixel_resampling_type'] != 'none':
            ...
            if self.config['pixel_resampling_type'] in ['lambda', 'lambda_shifted']:
                ...
                print("Δlog λ (constant) = {:.2e}".format(data_dict['pixelResampling']))
    ...
    #################################################################################################################
        ...
        print(f"Processing complete. The array containing the processed spectra has been stored in the HDF5 file {hdf5_path}.")
...
if __name__ == "__main__":
    ...
    # Convert CLI overrides into dict
    overrides = {}
    for i in range(0, len(unknown), 2):
        key = unknown[i].lstrip("-")
        value = yaml.safe_load(unknown[i + 1])
    ...
    """
# ------------- README -------------    
    # from shell: python path-to-file.stacking.py --config path-to-config/YAML/prova.yaml
    # from Jupyter notebook: 
    # import spectraPyle.stacking.stacking as smain
    # stack = smain.main(config)
